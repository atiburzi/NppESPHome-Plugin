...
type
  TComExceptions = (CE_OpenFailed, CE_WriteFailed, CE_ReadFailed, CE_InvalidAsync, CE_PurgeFailed,
    CE_AsyncCheck, CE_SetStateFailed, CE_TimeoutsFailed, CE_SetupComFailed, CE_ClearComFailed,
    CE_ModemStatFailed, CE_EscapeComFailed, CE_TransmitFailed, CE_ConnChangeProp,
    CE_EnumPortsFailed, CE_StoreFailed, CE_LoadFailed, CE_RegFailed, CE_LedStateFailed,
    CE_ThreadCreated, CE_WaitFailed, CE_HasLink, CE_RegError, CE_PortNotOpen, CE_PortDisconnected);

  // various types
  TPort = string;

  TBaudRate = (brCustom, br110, br300, br600, br1200, br2400, br4800, br9600, br14400, br19200,
...
begin
  if (FControlRTS = rtsHandshake) and (FOutCTSFlow) and (not FXonXoffIn) and (not FXonXoffOut) then
    ...
    Result := fcSoftware
  else if (FControlRTS = rtsDisable) and (not FOutCTSFlow) and (not FXonXoffIn) and (not FXonXoffOut)
  then
    Result := fcNone
  else
    Result := fcCustom;
...
begin
  ...
  begin
    ...
    FOnException(Self, TComExceptions(AnException), ComErrorMessages[AnException], WinError,
      WinMessage);
  end
...
begin
  ...
  if not FConnected and not(csDesigning in ComponentState) and (FPort <> '') then
  begin
    CallBeforeOpen;
...
const
  CParityBits: array [TParityBits] of Integer = (NOPARITY, ODDPARITY, EVENPARITY, MARKPARITY,
    SPACEPARITY);
  CStopBits: array [TStopBits] of Integer = (ONESTOPBIT, ONE5STOPBITS, TWOSTOPBITS);
  CBaudRate: array [TBaudRate] of Integer = (0, CBR_110, CBR_300, CBR_600, CBR_1200, CBR_2400,
  ...
  CControlRTS: array [TRTSFlowControl] of Integer = (RTS_CONTROL_DISABLE shl 12,
    RTS_CONTROL_ENABLE shl 12, RTS_CONTROL_HANDSHAKE shl 12, RTS_CONTROL_TOGGLE shl 12);
...
function TCustomComPort.WriteAnsiStrAsync(var Str: AnsiString; var AsyncPtr: PAsync): Integer;
begin
  if Length(Str) > 0 then
    Result := WriteAsync(Str[1], Length(Str), AsyncPtr)
  else
...
function TCustomComPort.ReadAnsiStr(var Str: AnsiString; Count: Integer): Integer;
var
  AsyncPtr: PAsync;
begin
  InitAsync(AsyncPtr);
...
procedure TCustomComPort.WaitForEvent(var Events: TComEvents; StopEvent: THandle; Timeout: Integer);
var
  Overlapped: TOverlapped;
  Mask: DWORD;
  Success: Boolean;
...
begin
  ...
  begin
    ...
    FlowControl.FlowControl := StrToFlowControl(IniFile.ReadString(Name, 'FlowControl',
  ...
  begin
    ...
    Parity.ReplaceChar := StrToChar(IniFile.ReadString(Name, 'Parity.ReplaceChar',
  ...
  begin
    ...
    Timeouts.ReadTotalConstant := IniFile.ReadInteger(Name, 'Timeouts.ReadTotalConstant',
      Timeouts.ReadTotalConstant);
    Timeouts.ReadTotalMultiplier := IniFile.ReadInteger(Name, 'Timeouts.ReadTotalMultiplier',
      Timeouts.ReadTotalMultiplier);
    Timeouts.WriteTotalConstant := IniFile.ReadInteger(Name, 'Timeouts.WriteTotalConstant',
      Timeouts.WriteTotalConstant);
    Timeouts.WriteTotalMultiplier := IniFile.ReadInteger(Name, 'Timeouts.WriteTotalMultiplier',
      Timeouts.WriteTotalMultiplier);
  end;
  if spFlowControl in FStoredProps then
  begin
    FlowControl.ControlRTS := StrToRTS(IniFile.ReadString(Name, 'FlowControl.ControlRTS',
      RTSToStr(FlowControl.ControlRTS)));
    FlowControl.ControlDTR := StrToDTR(IniFile.ReadString(Name, 'FlowControl.ControlDTR',
      DTRToStr(FlowControl.ControlDTR)));
    FlowControl.DSRSensitivity := StrToBool(IniFile.ReadString(Name, 'FlowControl.DSRSensitivity',
      BoolToStr(FlowControl.DSRSensitivity)));
    FlowControl.OutCTSFlow := StrToBool(IniFile.ReadString(Name, 'FlowControl.OutCTSFlow',
      BoolToStr(FlowControl.OutCTSFlow)));
    FlowControl.OutDSRFlow := StrToBool(IniFile.ReadString(Name, 'FlowControl.OutDSRFlow',
      BoolToStr(FlowControl.OutCTSFlow)));
    FlowControl.TxContinueOnXoff :=
      StrToBool(IniFile.ReadString(Name, 'FlowControl.TxContinueOnXoff',
      BoolToStr(FlowControl.TxContinueOnXoff)));
    FlowControl.XonXoffIn := StrToBool(IniFile.ReadString(Name, 'FlowControl.XonXoffIn',
      BoolToStr(FlowControl.XonXoffIn)));
    FlowControl.XonXoffOut := StrToBool(IniFile.ReadString(Name, 'FlowControl.XonXoffOut',
      BoolToStr(FlowControl.XonXoffOut)));
    FlowControl.XoffChar := StrToAnsiChar(IniFile.ReadString(Name, 'FlowControl.XoffChar',
      AnsiCharToStr(FlowControl.XoffChar)));
    FlowControl.XonChar := StrToAnsiChar(IniFile.ReadString(Name, 'FlowControl.XonChar',
      AnsiCharToStr(FlowControl.XonChar)));
  end;
end;
...
function BaudRateToStr(BaudRate: TBaudRate): string;
const
  BaudRateStrings: array [TBaudRate] of string = ('Custom', '110', '300', '600', '1200', '2400',
